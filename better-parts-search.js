...
(function () {
    ...
    var params = (new URL(document.location)).searchParams,
        search = params.get('s'),
        details = params.get('modal'),
        ready = false;

    if (search)
        $(document).arrive(".js-part-search-input", function () {
            // 'this' refers to the newly created element
            console.log('Searching');
            $(this).val(search);
            $('button', '#parts-page-search').click();
        });

    if (details)
        $(document).arrive("#parts_table", function () {
            // 'this' refers to the newly created element
            if (ready && $('.clickable_row', $(this).length)) {
                console.log('Ready');
                $(document).unbindArrive("#parts_table");
            }

            if (ready) {
                console.log('Details');
                $('.clickable_row', $(this)).eq(0).click()
            }
